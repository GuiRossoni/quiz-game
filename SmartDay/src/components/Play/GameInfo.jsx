...
export default function GameInfo () {
	...
	const mode = queries.timemode && queries.infinitymode ? 'Tempo | Infinito' : !queries.timemode && !queries.infinitymode ? 'Clássico' : queries.timemode ? 'Tempo' : 'Infinito'
		...
		<>
			...
			<aside className={`fixed h-fit transition-all z-10 lg:bottom-4 left-4 md:top-1/2 md:-translate-y-1/2 text-center text-slate-900 font-medium lg:!scale-100 lg:!opacity-100 ${showInfo ? 'bottom-12 scale-100 opacity-100' : 'bottom-0 scale-20 opacity-0'}`}>
				<div className='flex gap-2'>
					{
						!queries.infinitymode && <span className='bg-white p-2 rounded-md text-sm w-full pt-[9px] grid place-items-center' title='Número de perguntas'>
							{queries.questions}
						</span>
					}
					{
						queries.timemode && <span className='bg-white p-2 rounded-md text-sm w-full pt-[9px] grid place-items-center' title='Tempo'>
							{queries.time}
						</span>
					}
				</div>
				<div className='bg-white p-2 rounded-md mt-2 flex justify-center gap-1 items-center' title='Modo'>
