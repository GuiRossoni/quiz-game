...
export default function GameOver () {
	...
	function finalTitle () {
		if (queries.infinitymode) return 'Parabéns!'
		if (win === true) return 'Você Venceu!'
		return 'Você Perdeu!'
	}

	function finalText () {
		if (queries.infinitymode) return `Você respondeu bem ${score} perguntas!`
		if (win === true) return 'Parabéns! \nDesafio concluído com sucesso.'
				...
				<div className='flex flex-col items-center gap-4'>
					...
					<div className='flex gap-6 items-center'>
						...
						<button onClick={() => document.getElementById('newGameDialog').showModal()} className='btn-primary px-5 md:px-10 py-3 uppercase tracking-widest rounded-md bg-blue-500 text-white'>
							{queries.infinitymode || win !== false ? 'Mais uma?' : 'Agora vai!'}
						</button>
					</div>
				</div>
			</dialog>
